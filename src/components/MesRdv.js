...
class MonRdv extends React.Component {
    ...
    handleClick = () => {
        alert(
            ...
            "\n- Afficher le détail du rendez-vous avec Reservation.read et l'id " + this.props.rdv.id +
            "\n- Annuler le rendez-vous avec Reservation.annuler et l'id du RDV " + this.props.rdv.id +
            "\n- Modifier le rendez-vous avec Reservation.update et l'id du RDV " + this.props.rdv.id  + 
    ...
    render() {
        ...
        return (
            ...
            >
            { this.props.rdv.startAt }
            </Button>
            <br/>{ this.props.rdv.description }
            <Divider/>
            </div>
        )
    }
...
export default class MesRdv extends React.Component {
    ...
    render() {
        ...
        if (this.state.nouveauRdv) {
            return (
                <PriseRdv patient={ this.props.patient } identified={ true } client={ this.props.client } />
            );
        }
        // Sinon
        ...
        return (
            ...
            <Header size='medium'>
                {
                    this.state.mesRdv.length === 0 ? "Aucun rendez-vous"
                    :
                    this.state.mesRdv.length === 1 ? 'Mon prochain rendez-vous' : 'Mes prochains rendez-vous'
                }
            </Header>
             <Divider/>
            {   
